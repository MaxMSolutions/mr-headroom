document.addEventListener('DOMContentLoaded', () => {
    ...
    // Journal text files
    const journal1Icon = document.getElementById('journal1-icon');
    const journal2Icon = document.getElementById('journal2-icon');
    const journal3Icon = document.getElementById('journal3-icon');
    const journal4Icon = document.getElementById('journal4-icon');
    const journal5Icon = document.getElementById('journal5-icon');
    const journal6Icon = document.getElementById('journal6-icon');
    ...
    function createCrypticMessageWindow(message) {
        ...
        // Center the window
        const windowWidth = 300; // Set a fixed width
        const windowHeight = 150; // Approximate height
        messageWindow.style.width = `${windowWidth}px`;
        messageWindow.style.left = `${(window.innerWidth - windowWidth) / 2}px`;
        messageWindow.style.top = `${(window.innerHeight - windowHeight) / 2}px`;
    ...
    function createTextFileWindow(title, content) {
        const textWindow = document.createElement('div');
        textWindow.className = 'window';
        textWindow.innerHTML = `
            ...
            <div class="window-content" style="white-space: pre-wrap; font-family: 'Courier New', monospace; padding: 15px; height: 300px; overflow-y: auto;">
                ${content}
            </div>
        `;
        ...
        textWindow.style.width = `${windowWidth}px`;
        textWindow.style.height = `${windowHeight}px`;
        textWindow.style.left = `${(window.innerWidth - windowWidth) / 2}px`;
        textWindow.style.top = `${(window.innerHeight - windowHeight) / 2}px`;
        
        // Make it draggable
        makeDraggable(textWindow);
        
        // Window controls
        textWindow.querySelector('.close-button').addEventListener('click', () => {
            textWindow.remove();
        });
        
        textWindow.querySelector('.minimize-button').addEventListener('click', () => {
            // Just a visual effect for now
            textWindow.style.opacity = '0.5';
            setTimeout(() => textWindow.style.opacity = '1', 300);
        });
        
        textWindow.querySelector('.maximize-button').addEventListener('click', () => {
            // Toggle between normal and maximized
            if (!textWindow.classList.contains('maximized')) {
                textWindow.dataset.prevWidth = textWindow.style.width;
                textWindow.dataset.prevHeight = textWindow.style.height;
                textWindow.dataset.prevLeft = textWindow.style.left;
                textWindow.dataset.prevTop = textWindow.style.top;
                
                textWindow.style.width = '100%';
                textWindow.style.height = `calc(100% - ${document.getElementById('taskbar').offsetHeight}px)`;
                textWindow.style.left = '0';
                textWindow.style.top = '0';
                textWindow.classList.add('maximized');
            } else {
                textWindow.style.width = textWindow.dataset.prevWidth;
                textWindow.style.height = textWindow.dataset.prevHeight;
                textWindow.style.left = textWindow.dataset.prevLeft;
                textWindow.style.top = textWindow.dataset.prevTop;
                textWindow.classList.remove('maximized');
            }
        });
    ...
    journal6Icon.addEventListener('dblclick', () => {
        ...
        setTimeout(() => {
            ...
            for (let i = 0; i < 30; i++) {
                ...
                glitchBlock.style.width = `${Math.random() * 200}px`;
                glitchBlock.style.height = `${Math.random() * 50}px`;
                glitchBlock.style.backgroundColor = `rgb(${Math.random() * 255}, 0, 0)`; // Red-tinted
                glitchBlock.style.opacity = Math.random() * 0.7;
                glitchOverlay.appendChild(glitchBlock);
            }
